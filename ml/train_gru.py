"""Train a Keras GRU per stack to forecast P95 latency 30s ahead.

Companion to train_lstm.py: identical pipeline (same lookback window, same
per-feature input scaling, same TRAIN-only target scaling, same gradient
clipping and metric schema) but the two stacked LSTM layers are replaced with
two stacked GRU layers. This turns the study into a deep-learning architecture
comparison (LSTM vs GRU) with XGBoost/Prophet as the classic baselines.

Architecture: 2 stacked GRU layers x 64 units, lookback LOOKBACK_STEPS,
dense head -> 1 output. Saves the model (models/gru_{stack}.keras) and a
train/val loss curve PNG (results/gru_loss_{stack}.png).
...
Usage:
    python train_gru.py [--stack go] [--all] [--epochs 40]
"""
...
class _TargetScaler:
    ...
    Feeding it raw makes the MSE gradient enormous and the GRU diverges to
...
def train_stack(stack: str, epochs: int = 40, batch_size: int = 32,
    ...
    if save:
        model.save(f"{config.MODELS_DIR}/gru_{stack}.keras")
        _plot_loss(stack, hist.history)

    return {"stack": stack, "model_tag": "gru", "model": model,
...
def _plot_loss(stack: str, history: dict) -> str:
    ...
    plt.title(f"GRU training — {stack}")
    plt.xlabel("epoch")
    plt.ylabel("MSE loss")
    plt.legend()
    plt.tight_layout()
    out = f"{config.RESULTS_DIR}/gru_loss_{stack}.png"
...
def main() -> int:
    ...
    if not _HAVE_TF:
        print("tensorflow/keras not installed — skipping GRU training.")
    ...
    for stack in stacks:
        try:
            ...
            continue
        m = r["metrics"]
        print(f"[{stack}] gru  RMSE={m['rmse']:.5f}s  "
